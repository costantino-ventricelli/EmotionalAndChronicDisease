...
class ConverterEmothawToPathological:
    ...
    def __convert_file_to_pathological(new_directories, old_directories):
        for i in range(len(old_directories)):
            ...
            # Richiamo il metodo che estrarrà i tasks da ogni cartella utente e li modificherà in modo da renderli compa-
    ...
    """
        ...
        scansiona le cartelle di di sessione del vecchio dataset, dalle quali estrae i tasks che poi saranno sottoposti a
    ...
    def __extract_session_from_users(new_directories, old_directories, user_paths):
        ...
        for j in range(len(user_paths)):
            ...
            if len(session) > 0:
                # Ottengo la lista dei tasks
                tasks = os.listdir(session[0])
                # Verifico che ci siano percorsi nella lista dei tasks
                if len(tasks) > 0:
                    ...
                    # Genero i percorsi per ogni file di tasks.
                    tasks = [os.path.join(session[0], task) for task in tasks]
                    # Avvio la conversione dei file di tasks
                    ConverterEmothawToPathological.__convert_file_emothaw_to_pathological(tasks, user_paths[j], user_id, collection)
                else:
                    # Rimuovo la directory dell'utente se non ci sono tasks per quell'utente.
                    print("Removing user: ", j)
                    os.rmdir(user_paths[j])

    """
        @:param paths_to_task: contiene i percorsi di tutti i tasks da scansionare.
        @:save_directory: contiene il percorso di salvataggio dell'utente.
        @:user_id: l'id dell'utente a cui appartiene il tasks.
    """
    @staticmethod
    def __convert_file_emothaw_to_pathological(paths_to_task, save_directory, user_id, collection):
        # Scansiono uno ad uno i tasks e avvio la conversione degli stessi per scriverli nella struttura dei file presenti
        # nel dataset Pathological.
        for task in paths_to_task:
            # Individuo il numero del tasks dal suo percorso.
            task_search = re.search(r'hw(.*?).svc', task)
            # Verifico che sia stato individuato il numero del tasks.
            if task_search:
                # Ottengo il nuovo suffisso per il tasks individuato.
                task_suffix = ConverterEmothawToPathological.__switch_task(int(task_search.group(1)))
                # Genero il nuovo nome del file di tasks.
                new_file_name = "c" + str(collection) + "_u" + str(collection) + str(user_id) + task_suffix + "txt"
                # Creo uno scrittore di file csv per scrivere i dati nel nuovo file di tasks.
                new_file = csv.writer(open(os.path.join(save_directory, new_file_name), 'w'), delimiter=' ', quotechar='"')
                # Apro in lettura il vecchio file di tasks.
    ...
    def __switch_task(item):
        ...
        return switcher.get(item, "Invalid tasks")
